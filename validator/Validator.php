...
class Validator {
    ...
    protected function applyRule($field, $rule, $value)
    {
        ...
        if($field === 'phone') {
            ...
            if($rule === 'phoneNumber' && !ctype_digit($formatValue)) {
            ...
            if($rule === 'phoneBetween' && (strlen($formatValue) < $ruleValueArray[0] || strlen($formatValue) > $ruleValueArray[1])) {
        ...
        if($rule === 'image' && !in_array(strtolower(pathinfo($value['name'], PATHINFO_EXTENSION)), $ruleValueArray)) {
        ...
        if ($rule === 'campaignOtherRequired') {
            if(isset($this->data['campaign']) && $this->data['campaign']) {
                if(in_array('other', $this->data['campaign']) && (is_null($value) || $value === '')) {
                    $this->addError($field, $customMessage ?: "This other " . $fieldName . " is required");
                } 
            }
        }
